...
to add-to-wheel [ node-who portion ]
  ifelse table:has-key? wheel node-who [
    table:put wheel node-who (table:get wheel node-who + portion)
  ] [ 
    table:put wheel node-who portion
  ]

  set wheel-portion wheel-portion + portion
end

to-report spin-wheel
  let winner random wheel-portion
  let keys array:from-list table:keys wheel
  let index 0

  while [ winner > 0 ] [
    let portion table:get wheel array:item keys index
    set winner winner - portion
    set index index + 1
  ]
